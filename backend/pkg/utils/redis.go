...
func (*_redis) Del(key string) {
	...
	if err != nil {
		GLogger.Error(REDIS_UTIL_ERR_PREFIX+"Del: ", zap.Error(err))
		panic(err)
	}
}
...
func (*_redis) Set(key string, value interface{}, expiration time.Duration) {
	...
	if err != nil {
		GLogger.Error(REDIS_UTIL_ERR_PREFIX+"Set: ", zap.Error(err))
		panic(err)
	}
}
...
func (*_redis) Incr(key string) {
	...
	if err != nil {
		GLogger.Error(REDIS_UTIL_ERR_PREFIX+"Incr: ", zap.Error(err))
		panic(err)
	}
}
...
func (*_redis) GetInt(key string) int {
	...
	// if err != nil {
	// 	GLogger.Error(REDIS_UTIL_ERR_PREFIX+"GetInt: ", zap.Error(err))
	// 	panic(err)
	// }
	return val
}
...
func (*_redis) SAdd(key string, members ...any) {
	...
	if err != nil {
		GLogger.Error(REDIS_UTIL_ERR_PREFIX+"SAdd: ", zap.Error(err))
		panic(err)
	}
}
...
func (*_redis) HIncrBy(key, field string, incr int64) {
	...
	if err != nil {
		GLogger.Error(REDIS_UTIL_ERR_PREFIX+"HIncrBy: ", zap.Error(err))
		panic(err)
	}
}
...
func (*_redis) ZincrBy(key, member string, incr float64) {
	...
	if err != nil {
		GLogger.Error(REDIS_UTIL_ERR_PREFIX+"ZincrBy: ", zap.Error(err))
		panic(err)
	}
}
